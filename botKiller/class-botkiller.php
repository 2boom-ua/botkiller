...
private function load_search_engine_ips() {
    ...
    if (false === $this->google_ips) {
        ...
        set_transient('bot_killer_google_ips_v2', $this->google_ips, DAY_IN_SECONDS);
    }
...
if (stripos($user_agent, 'ChatGPT-User') !== false && 
    ...
    if ($has_js || $has_cookies) {
        if ($log_ai_user && !get_transient('bot_killer_chatgpt_logged_' . md5($ip))) {
            ...
            set_transient('bot_killer_chatgpt_logged_' . md5($ip), true, 3600);
        }
        return false;
    }
...
// WhatsApp user - in-app browser (no JS/cookies required)
if (stripos($user_agent, 'WhatsApp/') !== false && stripos($user_agent, 'Preview') === false) {
    
    $rate_key = 'bot_killer_whatsapp_rate_' . md5($ip);
    $rate_count = (int) get_transient($rate_key);
    $current_time = time();
    
    // Get first request time
    $first_time_key = 'bot_killer_whatsapp_first_' . md5($ip);
    $first_time = get_transient($first_time_key);
    
    if ($first_time === false) {
        $first_time = $current_time;
        set_transient($first_time_key, $first_time, 60);
    }
    ...
    // Rules:
    // 5-10 requests in 5-10 seconds → allow
    // >10 requests → reject
    if ($rate_count > 10) {
        $this->log_json($ip, "WHATSAPP user - blocked (too many: {$rate_count} in {$time_span}s)", 'rejected', 'log-rejected');
        return false;
    }
    
    if ($rate_count >= 5 && $time_span <= 10) {
        $this->log_json($ip, "WHATSAPP user - allowed (high activity: {$rate_count} in {$time_span}s)", 'user_detected', 'log-cart-user');
    } else {
        if ($log_app_user) {
...
if (stripos($user_agent, 'Telegram') !== false && stripos($user_agent, 'TelegramBot') === false) {
    ...
    // =============================================
    // TIKTOK BOT - DNS VERIFICATION FIRST, THEN ASN
    // =============================================
    if (stripos($user_agent, 'TikTokBot') !== false || stripos($user_agent, 'TikTokSpider') !== false) {
        
        $dns_valid = false;
        $hostname = $this->reverse_dns_lookup($ip, 5);
        if ($hostname) {
            if (strpos($hostname, '.tiktokcdn.com') !== false ||
                strpos($hostname, '.byteoversea.com') !== false ||
                strpos($hostname, '.tiktok.com') !== false) {
                $forward_ips = $this->forward_dns_lookup($hostname, 5);
                if ($forward_ips && is_array($forward_ips) && in_array($ip, $forward_ips)) {
                    $dns_valid = true;
                }
        ...
        if ($dns_valid) {
            $this->log_json($ip, "TIKTOK bot - allowed (DNS verified)", 'bot_detected', 'log-cart-bot');
            return false;
        }
        
        // Fallback: перевірка ASN
        $asn_info = $this->get_asn_for_ip($ip);
        $asn = $asn_info ? $asn_info['asn'] : null;
        
        if ($asn && in_array($asn, ['396982', '45102', '16509'])) {
            $this->log_json($ip, "TIKTOK bot - allowed (ASN: {$asn})", 'bot_detected', 'log-cart-bot');
            return false;
        }
        
        $this->log_json($ip, "TIKTOK bot - suspicious (DNS failed, ASN: " . ($asn ?: 'unknown') . ")", 'suspicious', 'log-cart-user');
        return false;
    }
...
if ((stripos($user_agent, 'facebookexternalhit') !== false || stripos($user_agent, 'Facebot') !== false) && !$is_allowed_multi) {
...
// ========== AMAZONBOT ==========
if (stripos($user_agent, 'Amazonbot') !== false) {
    $hostname = $this->reverse_dns_lookup($ip, 5);
    ...
    if (stripos($user_agent, 'Google-Site-Verification') !== false) {
        ...
        $cart_bots = [
            ...
            'petalbot' => [
                'agents' => ['PetalBot'],
                'dns' => ['.petalsearch.com', '.huawei.com'],
                ...
                'require_dns_only' => false,
                'spoof_risk' => 'low',
                'description' => 'PetalBot — strict ASN + DNS verification'
            ],
            ...
            'openai' => [
                ...
                'require_dns_only' => false,
                'spoof_risk' => 'medium',
                'description' => 'OpenAI crawler — requires official IP ranges or ASN 8075'
            ],
            // =============================================
            // OPENAI BOTS - SAFE MODE (logging only, no blocking)
            // =============================================
            'gptbot' => [
                'agents' => ['GPTBot'],
                'ip_ranges' => $this->get_gptbot_ip_ranges(),
                'asn' => ['8075'],
                'type' => 'hybrid',
                'allow_js' => true,
                'require_verification' => true,
                'require_dns_only' => false,
                'spoof_risk' => 'medium',
                'description' => 'GPTBot — official OpenAI IP ranges (safe mode)',
            ],
            'chatgpt_user' => [
                'agents' => ['ChatGPT-User'],
                'ip_ranges' => $this->get_chatgpt_user_ip_ranges(),
                'asn' => ['8075'],
                'type' => 'hybrid',
                'allow_js' => true,
                'require_verification' => true,
                'require_dns_only' => false,
                'spoof_risk' => 'medium',
                'description' => 'ChatGPT-User — official OpenAI IP ranges (safe mode)',
            ],
            'oai_searchbot' => [
                'agents' => ['OAI-SearchBot'],
                'ip_ranges' => $this->get_oai_searchbot_ip_ranges(),
                'asn' => ['8075'],
                'type' => 'hybrid',
                'allow_js' => true,
                'require_verification' => true,
                'require_dns_only' => false,
                'spoof_risk' => 'medium',
                'description' => 'OAI-SearchBot — official OpenAI IP ranges (safe mode)',
            ],
            'perplexity' => [
            ...
            'ccbot' => [
                ...
                'require_dns_only' => false,
                'spoof_risk' => 'low',
                'description' => 'CCBot — requires DNS or ASN 16509'
            ],
            'applebot' => [
                'agents' => ['Applebot'],
...
private function get_facebook_ip_ranges() {
    ...
    // Статичний список Facebook IP ranges (з офіційної документації Meta)
    $ranges = [
        // IPv4 Facebook
        '31.13.24.0/21',
        '31.13.64.0/18',
...
private function get_gptbot_ip_ranges() {
    $cache_key = 'bot_killer_gptbot_ips_v3';
    $fallback = [
        '132.196.86.0/24', '172.182.202.0/25', '172.182.204.0/24',
        '172.182.207.0/25', '172.182.214.0/24', '172.182.215.0/24',
        '20.125.66.80/28', '20.171.206.0/24', '20.171.207.0/24',
        '4.227.36.0/25', '52.230.152.0/24', '74.7.175.128/25',
        '74.7.227.0/25', '74.7.227.128/25', '74.7.228.0/25',
        '74.7.230.0/25', '74.7.241.0/25', '74.7.241.128/25',
        '74.7.242.0/25', '74.7.243.128/25', '74.7.244.0/25'
    ];
    
    return $this->get_openai_ips_from_json(
...
private function get_chatgpt_user_ip_ranges() {
    $cache_key = 'bot_killer_chatgpt_user_ips_v3';
    
    return $this->get_openai_ips_from_json(
        'https://openai.com/chatgpt-user.json',
        $cache_key,
        []  // no fallback — if fails, empty array
    );
}

private function get_oai_searchbot_ip_ranges() {
    $cache_key = 'bot_killer_oai_searchbot_ips_v3';
...
private function get_openai_ips_from_json($url, $cache_key, $fallback = []) {
    ...
    $ranges = [];
    $body = $this->safe_api_request($url, ['timeout' => 10], 'openai_ips');

    if ($body !== false) {
        $json = json_decode($body, true);
        if (isset($json['prefixes']) && is_array($json['prefixes'])) {
    ...
    if (empty($ranges) && !empty($fallback)) {
        $ranges = $fallback;
        $this->log_json('SYSTEM', "OpenAI IP fetch failed for {$url} - using fallback", 'system', 'log-default');
    }

    set_transient($cache_key, $ranges, DAY_IN_SECONDS);
    return $ranges;
}
...
private function get_perplexity_ip_ranges() {
    ...
    $ranges = [];
    $response = wp_remote_get('https://www.perplexity.com/perplexitybot.json', [
        'timeout' => 10,
    ...
    // Fallback на випадок недоступності API
    if (empty($ranges)) {
        $ranges = [
            '107.20.236.150/32', '3.224.62.45/32', '18.210.92.235/32',
            '3.222.232.239/32', '3.211.124.183/32', '3.231.139.107/32',
            '18.97.1.228/30', '18.97.9.96/29'
        ];
    }
...
public function update_all_bot_ip_ranges($source = 'cron') {
    ...
    // OpenAI bots - safe mode
    $this->get_gptbot_ip_ranges();
    $this->get_chatgpt_user_ip_ranges();
    $this->get_oai_searchbot_ip_ranges();
    
    // Perplexity Bot
    delete_transient('bot_killer_perplexity_ips');
...
if (!$cart_bot) {
    ...
    if ($cart_bot) {
        ...
        if (in_array($cart_bot, ['gptbot', 'chatgpt_user', 'oai_searchbot'])) {
            $this->log_json($ip, "OPENAI BOT DETECTED: " . strtoupper($cart_bot) . " - logging only, no block", 'bot_detected', 'log-cart-bot');
            return $passed;
        }
        ...
        $social_hybrid_bots = ['facebook', 'whatsapp', 'linkedin', 'pinterest', 'twitter', 
                       ...
                       'gptbot', 'chatgpt_user', 'oai_searchbot'];  // додано для логування
...
private function check_browser_integrity_with_score($ip, $user_agent) {
    ...
    $allowed_countries = get_option('bot_killer_allowed_countries', array());
    $geo = $this->get_geo_location($ip);
    $country_code = is_array($geo) ? ($geo['country_code'] ?? '') : '';
    
    if (in_array($country_code, $allowed_countries, true)) {
        // Softer thresholds for allowed countries (trusted regions)
        $pass_threshold = $is_mobile ? 7 : 4;
        $reject_threshold = $is_mobile ? 15 : 6;
        $block_threshold = $is_mobile ? 18 : 7;
    } else {
        // Stricter thresholds for other countries (untrusted regions)
        $pass_threshold = $is_mobile ? 7 : 2;
        $reject_threshold = $is_mobile ? 15 : 4;
        $block_threshold = $is_mobile ? 18 : 5;
    }
...
private function get_azure_ip_ranges() {
    $cache_key = 'bot_killer_azure_ips';
    $ranges = get_transient($cache_key);
    ...
    $ranges = [
        ...
        '20.0.0.0/10',      // Azure US East, Europe
        '23.96.0.0/14',     // Azure US
        '23.98.0.0/15',     // Azure US
        '40.64.0.0/10',     // Azure
        '52.96.0.0/14',     // Azure
        '52.160.0.0/11',    // Azure US
        '52.224.0.0/11',    // Azure Europe
        '20.215.0.0/16',    // Azure Poland
        '20.245.0.0/16',    // Azure San Francisco
        '20.41.0.0/16',     // Azure UK
        '20.67.0.0/16',     // Azure
        '20.90.0.0/16',     // Azure
        '20.105.0.0/16',    // Azure
        '20.121.0.0/16',    // Azure
...
public function track_cart_item_quantity_update($cart_item_key, $quantity, $old_quantity) {
    ...
    if ($cart_bot) {
        ...
        if (in_array($cart_bot, $social_hybrid_bots)) {
            $this->log_json($ip, "BOT ABUSE: " . strtoupper($cart_bot) . " - cart update detected", 'spoof', 'log-spoof-attempt');
            $this->block_ip($ip, "BOT ABUSE: " . strtoupper($cart_bot) . " cart update", $cart_bot, 'bot_abuse_detected', 'spoof_bot');
            return;
        }
        
        // Strict bots (Google, Bing) - allow
        $this->log_bot_detection($ip, $cart_bot, $user_agent, 'detected_in_update');
        return;
    }
